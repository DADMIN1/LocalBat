...
# creates a dbfile and exits only if it doesn't exist
def EstablishDB():
    dbfile = DATABASE_PATH / "localbat.db"
    if dbfile.exists(): return;
    print(f"Establishing new database!")
    dbconn = sqlite3.connect(dbfile)
    with dbconn: dbconn.executescript(table_definitions);
    dbconn.close()
    print(f"created database at: {dbfile}")
    print("exiting")
    exit(0)
...
def Commit():
    EstablishDB() # exits here if it actually creates the database; otherwise has no effect
    statusCode = {
    ...
    WriteJavaDatabase() # updating timestamps
    return
...
def WriteJavaDatabase():
    ...
    with open(DATABASE_PATH / "Database.java", 'w', encoding="utf-8") as file:
        file.write("// the contents of this file are regenerated by 'database.py' on commit\n")
...
# This function doesn't need to exist anymore; the 'Validation.java' file is completely static now.
# Originally it required some code generation, but all of it was refactored into 'Database.java'. (see 'WriteJavaDatabase()' above)
...
# TODO: argparse to select operation
